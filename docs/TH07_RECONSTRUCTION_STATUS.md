...
- Source-derived / anchored: SHT path table/order, SHT callback addresses and high-level semantics, Player/Bomb routine tables, Player ANM VM prefix and collision field offsets, bomb score-drain/speed/common-effect metadata, Effect table/layout metadata, Stage camera/ANM VM opcode slices, clear-bonus constants, and Cherry/Border notification labels.
- Transitional: `src/Player.cpp`, `src/BombData.*`, `src/EffectManager.*`, `src/Stage.*`, and other flat TH06-style runtime files that now carry TH07 evidence slices but still need function-level replacement before gameplay parity.
...
| Player bomb start constants | `FUN_004083f0`, `FUN_004084f0`, `FUN_00408610`, `FUN_00408710..0040e280`, plus the `004083f0`/`00441330`/`004420b0` calls into `FUN_00406930`, decompiler output and PE instruction checks in `th07.exe` | duration/common-effect/score-drain/speed/backdrop/stage-color fields and helpers in `src/PlayerBombTables.*`; `DAT_004d44e0` is mapped as `DAT_004bdad8 + 0x16a08` |
...
| Area | Why Not Done |
| --- | --- |
| Player shot behavior | `.sht` binary records plus SHT/player callback table addresses, high-level callback semantics, the original `0x24c` ANM VM prefix layout, and the position/collision field slice are decoded. `Player.hpp` now carries the leading TH07 layout bridge, but the callback runtime, exact SHT-derived player-size initialization, and `Player.cpp` firing behavior still need TH07 function-level replacement. |
| Bomb behavior | TH07 focused/unfocused bomb routine slots, lifecycle fields, player object base/clear span, player ANM VM prefix layout, position/collision fields, common-effect durations, common start-effect spawn metadata, score-drain factors/minimums, score-drain-step math, common backdrop color timing/runtime color-write wiring, mode-4 stage-color write producers, mode transition effect metadata, and movement speed multipliers are decoded. The `+0x23f0/+0x23f4` speed multiplier offsets are now structurally placed in `Player.hpp`, but full calc/draw/update runtime replacement is still pending. |
| Effect manager | TH07's 34-entry effect table, slot size/counts, dedicated-spawn contract, draw-list routing, stage effect ANM loads, first callback slices, stage camera model, Stage ANM VM runtime slice, render-helper routing, `FUN_0044efb0` submit slice, `FUN_0044b0e0` shake draw offsets plus the mapped `DAT_0062f858` GameManager frame-counter gate, and the `FUN_00406930`/`004069d0`/`00406de0` color-multiplier path are anchored in portable metadata and PE/audit checks. The common bomb backdrop producer `004083f0` is wired through the Stage color writer and its `_DAT_0134cdb0 = 1` reset guard is mapped to `DAT_01347b00 + 0x52b0`; the mode-4 player producers and transition effect constants are mapped but not yet wired to the current Player runtime. Several TH07 callback bodies, exact batched `FUN_0044f690`/`FUN_0044f5c0` buffer flushing, exact x87/CRT edge-case parity for math/float comparisons, and the conditional RGB-white gate for `0041b0b0`/`0041b4a0` still need function-level replacement. |
...
Next proof step: decide whether the remaining renderer migration should reconstruct the original `FUN_0044f690`/`FUN_0044f5c0` batch buffer or keep the current immediate diffuse submission as a portability boundary. In parallel, continue the Player replacement from the new `0x24c` prefix/position bridge by proving or replacing original fields `+0x2408`, `+0x240a`, `+0x240d`, `+0x16a00..+0x16a14`, `+0xb7e68`, and `+0xb7e6c`, then wire the mapped mode-4 `FUN_00406930` producers and transition effects; `DAT_004d44e0` is known to be the original global-player `+0x16a08` alias. After that, map the score/cherry globals behind the conditional RGB-white gate in `0041b0b0`/`0041b4a0`.
