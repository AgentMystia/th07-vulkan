...
const outDir = join(root, 'reference/ghidra/decomp');
...
const args = existsSync(projectFile)
  ? [projectDir, 'th07', '-process', 'th07.exe', '-scriptPath', scriptDir, '-postScript', 'ExportTh07FullDecomp.java', outDir]
  : [projectDir, 'th07', '-import', exe, '-overwrite', '-scriptPath', scriptDir, '-postScript', 'ExportTh07FullDecomp.java', outDir];
...
if (result.status !== 0) {
  throw new Error(`Ghidra full decompilation export failed with status ${result.status}. Run npm run ghidra-export first if the project is missing or stale.`);
}
