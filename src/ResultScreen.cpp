...
DIFFABLE_STATIC_ARRAY_ASSIGN(char *, 6, g_CharacterList) = {"ReimuA", "ReimuB", "MarisaA", "MarisaB", "SakuyaA", "SakuyaB"};
...
ScoreDat *ResultScreen::OpenScore(char *path)
{
    ...
    else
    {
        ...
        while (fileLen > 0)
        {
            ...
            decryptedFilePointer = decryptedFilePointer->ShiftBytes(decryptedFilePointer->th7kLen);
            fileLen = fileLen - decryptedFilePointer->th7kLen;
        }
        if (fileLen <= 0)
        {
            free(scoreData);
...
u32 ResultScreen::GetHighScore(ScoreDat *scoreDat, ScoreListNode *node, u32 character, u32 difficulty)
{
    ...
    while (remainingSize > 0)
    {
        ...
        remainingSize -= highScore->base.th7kLen;
        highScore = highScore->ShiftBytes(highScore->base.th7kLen);
    }
...
ZunResult ResultScreen::ParseCatk(ScoreDat *scoreDat, Catk *outCatk)
{
    ...
    while (cursor > 0)
    {
        ...
        parsedCatk = (Catk *)&parsedCatk->name[parsedCatk->base.th7kLen - 0x18];
    }
    return ZUN_SUCCESS;
}
...
ZunResult ResultScreen::ParseClrd(ScoreDat *scoreDat, Clrd *outClrd)
{
    ...
    for (characterShotType = 0; characterShotType < CLRD_NUM_CHARACTERS; characterShotType++)
    {
        ...
        outClrd[characterShotType].base.th7kLen = sizeof(Clrd);
    ...
    while (cursor > 0)
    {
        ...
        parsedClrd = (Clrd *)((i32)&parsedClrd->base + parsedClrd->base.th7kLen);
    }
    return ZUN_SUCCESS;
}
...
ZunResult ResultScreen::ParsePscr(ScoreDat *scoreDat, Pscr *outClrd)
{
    ...
    while (cursor > 0)
    {
        ...
        parsedPscr = parsedPscr->ShiftBytes(parsedPscr->base.th7kLen);
    }
    return ZUN_SUCCESS;
}
#pragma intrinsic(memset)
...
void ResultScreen::WriteScore(ResultScreen *resultScreen)
{
    ...
    resultScreen->fileHeader.unkLen = sizeof(Th7k);
    resultScreen->fileHeader.th7kLen = sizeof(Th7k);
    resultScreen->fileHeader.version = TH6K_VERSION;

    memcpy(fileBuffer + sizeOfFile, &resultScreen->fileHeader, sizeof(Th7k));
...
i32 ResultScreen::HandleReplaySaveKeyboard()
{
    ...
    CHOOSE_REPLAY_FILE:
        ...
        if (this->frameTimer == 0)
        {
            ...
            {
                sprintf(replayToReadPath, "./replay/th7_%.2d.rpy", idx + 1);
    ...
    case RESULT_SCREEN_STATE_WRITING_REPLAY_NAME:
        ...
        if (WAS_PRESSED_WEIRD(TH_BUTTON_SELECTMENU))
        {
            ...
            {
                sprintf(replayPath, "./replay/th7_%.2d.rpy", this->replayNumber + 1);
...
ChainCallbackResult th07::ResultScreen::OnDraw(ResultScreen *resultScreen)
{
    AnmVm *sprite;
    ...
    for (i = 0; i < HSCR_NUM_DIFFICULTIES; i++)
    {
        for (characterShot = 0; characterShot < HSCR_NUM_CHARS_SHOTTYPES; characterShot++)
        {
            for (slot = 0; slot < HSCR_NUM_SCORES_SLOTS; slot++)
            {
                ...
                resultScreen->defaultScore[i][characterShot][slot].base.th7kLen = sizeof(Hscr);
